...
namespace eosiosystem {
   ...
   void system_contract::regproducer( const account_name producer, const eosio::public_key& producer_key, const std::string& url, uint16_t location ) {
      ...
      std::cout<<eosio::public_key()<<std::endl;
      require_auth( producer );

      auto prod = _producers.find( producer ); //find 查找

      if ( prod != _producers.end() ) { //存在
         _producers.modify( prod, producer, [&]( producer_info& info ){ //修改
         ...
         _producers.emplace( producer, [&]( producer_info& info ){  //增加
               info.owner         = producer;
               info.total_votes   = 0;
               info.producer_key  = producer_key;
               info.is_active     = true;
   ...
   void system_contract::unregprod( const account_name producer ) {
      ...
      const auto& prod = _producers.get( producer, "producer not found" ); //实际调用find方法
   ...
   void system_contract::update_elected_producers( block_timestamp block_time ) {
      _gstate.last_producer_schedule_update = block_time; //更新时间

      auto idx = _producers.get_index<N(prototalvote)>(); //获得索引, 按照投票数排序

      std::vector< std::pair<eosio::producer_key,uint16_t> > top_producers;
      top_producers.reserve(21); //vector预分配存储区大小
      ...
      if ( top_producers.size() < _gstate.last_producer_schedule_size ) {//生产者比之前的个数小，这个更新无效？
         return;
      }

      /// sort by producer name
      std::sort( top_producers.begin(), top_producers.end() ); //按账户名排名(所以这时候票数多少没有意义了?)，是不是账户名好的有优势？

      std::vector<eosio::producer_key> producers;

      producers.reserve(top_producers.size());
      for( const auto& item : top_producers )
         producers.push_back(item.first); //提取出producer_key结构单独塞进producers

      bytes packed_schedule = pack(producers);

      if( set_proposed_producers( packed_schedule.data(),  packed_schedule.size() ) >= 0 ) {
         _gstate.last_producer_schedule_size = static_cast<decltype(_gstate.last_producer_schedule_size)>( top_producers.size() ); //更新这次producer的size
      }
   }
