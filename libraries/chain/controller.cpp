...
int64_t controller::set_proposed_producers( vector<producer_key> producers ) {
   ...
   auto cur_block_num = head_block_num() + 1; //下一个区块高度

   if( gpo.proposed_schedule_block_num.valid() ) {
      if( *gpo.proposed_schedule_block_num != cur_block_num ) //已经有
         return -1; // there is already a proposed(被提议的) schedule(日程安排) set in a previous block, wait for it to become pending

      if( std::equal( producers.begin(), producers.end(),
                      gpo.proposed_schedule.producers.begin(), gpo.proposed_schedule.producers.end() ) ) //producer没改变
         return -1; // the proposed producer schedule does not change
   }

   producer_schedule_type sch;

   decltype(sch.producers.cend()) end; //decltype
   decltype(end)                  begin;
