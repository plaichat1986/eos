...
class privileged_api : public context_aware_api {
   public:
      ...
      int64_t set_proposed_producers( array_ptr<char> packed_producer_schedule, size_t datalen) {
         ...
         fc::raw::unpack(ds, producers); //解出vector<producer_key>(为什么前面pack这里又unpack)

         //max_producers:125 (不会超过21，怎么还需要125的限制)
         ...
         for (const auto& p: producers) {
            ...
            unique_producers.insert(p.producer_name); //放入std::set(去重)
         }

         //如果传过来的producers里面包含相同的账户，会导致unique_producers的size小于producers的size
         //为啥不直接用set结构，省的这里还需要去重处理?
